...
public class StreamGroupingByDemo {
  public static void main(String[] args) {
    List<Product> productList = Arrays.asList(
        ...
        new Product("Nokia", 800), new Product("BlackBerry", 1000),
        new Product("Apple Pro Max", 1500),
        new Product("MI", 800),
        new Product("OnePlus", 1000)
    );

    Map<Integer, List<Product>> groupingByPriceMap = productList.stream().collect(Collectors.groupingBy(Product::getPrice));

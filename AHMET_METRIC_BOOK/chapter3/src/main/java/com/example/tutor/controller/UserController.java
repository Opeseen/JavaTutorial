...
import com.example.tutor.exceptions.EmailUniquenessExceptions;
...
public class UserController {
  ...
  public Mono<ResponseEntity<User>> createUser(@RequestBody User user){
    return userRepository.findByEmail(user.email())
        .flatMap(existingUser -> Mono.error(new EmailUniquenessExceptions("Email already exists")))
        ...
        .doOnNext(saveUser -> System.out.println("New user created: " + saveUser)) // Logging or further action
        .onErrorResume(e -> { // Handling errors, such as email uniqueness violation
          System.out.println("An exception has occurred: " + e.getMessage());
          if(e instanceof EmailUniquenessExceptions){

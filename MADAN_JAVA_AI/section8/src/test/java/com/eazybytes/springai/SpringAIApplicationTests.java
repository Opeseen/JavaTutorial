...
class SpringAIApplicationTests {
  ...
  void evaluateFactAccuracyForGravityQuestion() {
    // Given
    String question = "Who discover the law of universal gravitation?";
    // When
    String aiResponse = chatController.chat(question);
    EvaluationRequest evaluationRequest = new EvaluationRequest(question, aiResponse);
    ...
    Assertions.assertAll(() -> assertThat(aiResponse).isNotBlank(),
        () -> assertThat(response.isPass())
            .withFailMessage("""
                                ========================================
                                The answer was not considered factually relevant.
                                Question: "%s"
                                Response: "%s"
                                ========================================
                                """, question, aiResponse)
  ...
  void evaluateHrPolicyAnswerWithRagContext() throws IOException {
    // Given
    String question = "How many paid leave do employee get annually?";
    // When

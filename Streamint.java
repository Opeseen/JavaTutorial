  ...
  public static void main(String[] args){
   List<Integer> list = Arrays.asList(5, 5, 3, 4, 5);
    ...
    // filter by number divisible by 5 and 7
		...
		// stream.filter(i -> (i % 5 == 0 && i % 7 == 0)).forEach();
    ...
    // System.out.println("IntStream to Set : "+set);
		...
		List<Integer> Listed = new ArrayList<>(Arrays.asList(integers));
		// System.out.println(list22.get(4));
		IntStream stream = IntStream.range(0, Listed.size());
		// OptionalInt result = stream.filter(index -> Listed.get(index).equals(4)).findFirst();
		OptionalInt result = stream.filter(index -> Listed.get(index).equals(5)).findFirst();
		System.out.println("result is " + result);
  }
}